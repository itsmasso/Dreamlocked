		...
		foreach (Room room in houseMapGenerator.GetNormalRoomComponents().ToList())
		{
			foreach (Transform monsterTransform in room.monsterTransforms)
			{
				if (mannequinSpawnCount <= currentDifficultySetting.mannequinSpawnAmount)
				{
					...
					{
						SpawnMonster(MQMonsterPrefab, new Vector3(monsterTransform.position.x, monsterTransform.position.y + MQMonsterPrefab.GetComponentInChildren<CapsuleCollider>().height / 16 + 0.25f, monsterTransform.position.z), Quaternion.identity);
						mannequinSpawnCount++;
					}
